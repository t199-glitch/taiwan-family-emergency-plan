...
$parser = New-Object Microsoft.VisualBasic.FileIO.TextFieldParser("google_errata_utf8.csv", $utf8)
...
foreach ($c in $corrections) {
    ...
    # Match by ID or Name (skipping index 0)
    $matched = $null
    ...
    # If name didn't match, match by ID
    if (-not $matched -and $c.id -gt 0 -and $c.id -le $shelters.Count) {
        $candidate = $shelters[$c.id - 1]
        $matched = $candidate
    }

    if ($matched) {
        Write-Host ("UPDATED [{0}] Coords: ({1}, {2}) -> ({3}, {4})" -f $matched.name, $matched.lat, $matched.lng, $c.lat, $c.lng)
        $matched.lat = $c.lat
        $matched.lng = $c.lng
