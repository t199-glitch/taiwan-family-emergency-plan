...
$csv = Import-Csv -Path "shelters_utf8.csv" -Encoding UTF8

foreach ($row in $csv) {
    $c_county = $row.psobject.properties['縣市'].Value
    if ([string]::IsNullOrWhiteSpace($c_county)) { $c_county = $row.psobject.properties['縣市名稱'].Value }
    
    $c_district = $row.psobject.properties['鄉鎮市區'].Value
    $c_name = $row.psobject.properties['避難收容處所名稱'].Value
    $c_addr = $row.psobject.properties['避難收容處所地址'].Value
    $c_cap = $row.psobject.properties['預估收容人數'].Value
    
    $lngStr = $row.psobject.properties['經度'].Value
    $latStr = $row.psobject.properties['緯度'].Value
...
Write-Host "Total clean parsed CSV items:" $list.Count
...
Write-Host "Successfully generated clean shelters_data.js!"
